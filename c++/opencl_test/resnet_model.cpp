...
BottleneckBlock::BottleneckBlock(const ClientSession &session, const Scope &scope, Input input, vector<Tensor> &filters,
                                 Output min_input, Output max_input,
    ...
    //Conv 1
    ConvPadding Conv_1(session, scope, input, filters[0], min_input, max_input, min_filter[0], max_filter[0], 1, 1, world);
    ...
    ConvPadding Conv_2(session, scope, QRelu_1.activations, filters[1], QRelu_1.min_activations,
                       ...
                       min_filter[1], max_filter[1], 3, strides, world);
    ...
    ConvPadding Conv_3(session, scope, QRelu_2.activations, filters[2], QRelu_2.min_activations,
                       ...
                       min_filter[2], max_filter[2], 1, 1, world);
...
BlockLayer::BlockLayer(const ClientSession &session, const Scope &scope, Input input, vector<vector<Tensor>> &filters,
                       Output min_input, Output max_input,
    ...
    for (int i = 1; i < blocks; i++) {
        BottleneckBlock block_i(session, scope, block_output, filters[i], block_output_min, block_output_max,
                                min_filter[i], max_filter[i], param[i], false, 1, world);
